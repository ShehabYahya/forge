...
For OpenCode, register exactly one MCP entry and one plugin entry. Point the MCP command at `forge-alpha` and the plugin entry at `forge/plugin/opencode/dist/index.js` from the installed package or source checkout. The plugin config hook registers `/review-memory` automatically, so do not copy a command file into each repository. Do not register both global and project-local copies of either entry; OpenCode starts one stdio MCP process for each resolved registration.
