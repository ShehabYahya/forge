...
| H27 | forge/plugin/opencode/src/compaction.ts:269 | bug | TOCTOU race condition in assertSafePath: symlink check and subsequent readFile are not atomic |
| H28 | forge/plugin/opencode/src/compaction.ts:269 | bug | Synchronous fs calls (lstatSync, realpathSync) in async methods block the event loop |
...
| M58 | forge/plugin/opencode/src/index.ts:154 | bug | recoverFullOutput hardcodes Linux XDG path; may silently fail on macOS/Windows |
...
| M62 | README.md:235 | readme_drift | README 'Output compaction' feature row does not mention forge_expand_tool_result tool |
...
| M64 | forge/plugin/opencode/src/compaction.ts:269 | bug | assertSafePath can crash with unhandled ENOENT if the file or root directory is deleted between reads |
| M65 | forge/plugin/opencode/src/transcript.ts:52 | naming_drift | sessionID (capital D) in transcript.ts vs sessionId (lowercase d) in compaction.ts for same logical concept |
...
| M72 | README.md:221 | readme_drift | Program/executable resolution uses FORGE_PROGRAM; compaction runtime uses FORGE_HOME. Neither is fully documented. |
...
| L91 | forge/plugin/opencode/src/compaction.ts:215 | other | Hardcoded limit of 20 matches in search() is not configurable |
...
| S28 | issues | 8 | forge/plugin/opencode/src/compaction.ts + forge/plugin/opencode/src/transcript.ts |
...
> The MCP tool signatures in mcp_server.py largely mirror the service methods in service.py, but one significant gap exists: scope_mode is not exposed. The service supports 'strict' and 'warning' (service.py:62), but the MCP wrapper hard-codes 'strict' by omission. All other tool signatures (review_changes, finish_task, submit_outcome, expand_tool_result) are fully consistent between the two files. Version (0.1.0-alpha.1, in forge/__init__.py:3) matches README. Runtime root defaults to ~/.forge/ with FORGE_HOME/ FORGE_ALPHA_HOME env overrides (service.py:31-34) matching README claims. The README and plugin forge-system.ts consistently use a forge_ prefix for tool names (forge_start_task, forge_review_changes, forge_finish_task, forge_submit_outcome, forge_expand_output), but the actual MCP tool names registered by mcp_server.py lack this prefix (start_task, review_changes, finish_task, submit_outcome, expand_tool_result). Additionally, forge-system.ts:119 references forge_expand_output as a distinct tool, but mcp_server.py only registers expand_tool_result — no expand_output tool exists in either assigned file.
...
README/INSTALL DRIFT: README.md's command table (lines 211-219) lists CLI commands (forge install, forge doctor, forge mcp, etc.) which are not present in either assigned TypeScript file — they live in the Python backend. The README's 'Output compaction' capability row (line 235) describes both forge_expand_output and forge_expand_tool_result as a single feature, but forge-system.ts defines them as separate tools with distinct purposes. INSTALL.md's plugin dev instructions (lines 72-82) are consistent with the TypeScript code — there is no drift vs the assigned files.
...
FORGE_ALPHA_HOME legacy env var (README line 222) is referenced in compaction.ts but not in governor.ts or forge-system.ts — no drift in the assigned files.

**S28** (forge/plugin/opencode/src/compaction.ts + forge/plugin/opencode/src/transcript.ts):
> compaction.ts and transcript.ts serve distinct purposes (output storage/retrieval vs session evidence tracking) with no code duplication. They are consistent in their overall session-concept usage. Key inconsistencies: (1) sessionId vs sessionID casing — compaction.ts uses lowercase 'd', transcript.ts uses uppercase 'D'; index.ts bridges both conventions. (2) Both files use snake_case for TypeScript type properties (StoredMetadata in compaction.ts, SessionDigest in transcript.ts), diverging from the camelCase convention used elsewhere in the plugin. (3) Both files correctly export their classes and types, and both have corresponding test files (compaction.test.ts, transcript.test.ts). (4) No drift found against README claims: runtimeRoot() implements FORGE_HOME/FORGE_ALPHA_HOME (README L221-222), compaction implements 'expandable line-range handles' (README L235), and transcript implements session evidence tracking implied by 'finish receipts' (README L83-84). (5) INSTALL.md plugin dev instructions (npm install, npm run typecheck, npm test, npm run build) are not contradicted by this code.
