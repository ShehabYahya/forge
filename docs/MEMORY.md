...
Forge Alpha memory now has two distinct paths:
...
Runtime memory lives under `~/.forge-alpha/memory/`:
...
Configuration overrides live in `~/.forge-alpha/config.json`. Memory maintenance settings use the nested `memory.maintenance.review` object; the earlier `memory.maintenance_review` spelling remains accepted for compatibility.
