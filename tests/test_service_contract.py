...
def test_every_response_has_envelope(service, repo):
    responses = [
        service.start_task("", str(repo)),
        service.start_task("task", str(repo)),
        service.review_changes("missing"),
        service.finish_task("missing", False, "x"),
        service.submit_outcome(False, "x", "reason", task_id="missing"),
        service.expand_tool_result("missing", "fr_" + "0" * 32),
    ]
...
def test_invalid_repository_and_escaping_expected_path_rejected(service, tmp_path, repo):
    assert not service.start_task("task", str(tmp_path / "missing"))["ok"]
    result = service.start_task("task", str(repo), ["../escape"])
...
def test_bound_session_is_idempotent_unless_replaced(tmp_path, repo):
    ...
    first = service.start_task("first", str(repo), host_session_id="session")
    same = service.start_task("second", str(repo), host_session_id="session")
    assert same["task_id"] == first["task_id"] and same["idempotent"]
    replacement = service.start_task("second", str(repo), host_session_id="session", replace_active=True)
